...
class RobotMe(object):
    ...
    def reset(self, deterministic=False):
        ...
        self.sim.data.qpos[self._ref_joint_pos_indexes[:7]] = init_qpos[:7]
    ...
    def setup_observables(self):
        ...
        def joint_pos(obs_cache):
            return np.array([self.sim.data.qpos[x] for x in self._ref_joint_pos_indexes])[:7]

        @sensor(modality=modality)
        def joint_pos_cos(obs_cache):
            return np.cos(obs_cache[pre_compute]) if pre_compute in obs_cache else np.zeros(self.robot_model.dof)[:7]

        @sensor(modality=modality)
        def joint_pos_sin(obs_cache):
            return np.sin(obs_cache[pre_compute]) if pre_compute in obs_cache else np.zeros(self.robot_model.dof)[:7]

        @sensor(modality=modality)
        def joint_vel(obs_cache):
            return np.array([self.sim.data.qvel[x] for x in self._ref_joint_vel_indexes])[:7]
    ...
    def torque_limits(self):
        ...
        low = self.sim.model.actuator_ctrlrange[self._ref_joint_actuator_indexes, 0][:7]
        high = self.sim.model.actuator_ctrlrange[self._ref_joint_actuator_indexes, 1][:7]
