...
class OperationalSpaceController(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self._ARM_XML = "/homeL/wchen/Assembly_RPL/robosuite/models/assets/robots/lwr/robot.xml"
        self.physics = mujoco.Physics.from_xml_path(self._ARM_XML)
        #pdb.set_trace()
        self.site_name = 'ft_frame'  
        #'grip_site' 
    ...
    def run_controller(self):
        ...
        # covert Eular to Quaternion
        #desired_ori_Q_temp =  T.euler2mat(desired_ori)  # first check desired_ori is mat or euler !
        desired_ori_Q =  T.mat2quat(desired_ori)

        
        target_pos = np.array(desired_pos).copy()
        target_quat = np.array(desired_ori_Q).copy()
        ...
        desired_joint_angles = result[0][:7]  #  MoveIT ?? 

        # PD-like impedance control in joint space
        joint_angle_err = desired_joint_angles - self.joint_pos  # make sure desired_joint_angles is np.array-like from IK
        joint_vel_err = -self.joint_vel
        Kp_q = np.array([800, 800, 800, 800, 800, 800, 800])
        Kd_q = np.sqrt(Kp_q)

        self.torques = np.multiply(Kp_q, joint_angle_err) + np.multiply(Kd_q, joint_vel_err) + self.torque_compensation
