...
class OperationalSpaceController(Controller):
    ...
    def __init__(
        ...
        joint_indexes,
        actuator_range,
        input_max=(1, 1, 1, 0.05, 0.05, 0.05),
        input_min=(-1, -1, -1, -0.05, -0.05, -0.05),
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.J_full_last = np.array([[ 2.10922479e-02,  1.50641611e-02,  1.86550013e-02,
                                        3.43489217e-01,  3.04927615e-03, -7.78853704e-02,
                                        0.00000000e+00],
                                    [ 4.62005356e-01, -6.11924634e-04,  4.07886677e-01,
                                        -1.52449688e-02,  5.66370246e-02,  4.19544348e-03,
                                        0.00000000e+00],
                                    [-0.00000000e+00, -4.62480740e-01, -1.02567215e-03,
                                        2.85885035e-01,  2.38729626e-04, -5.17041499e-04,
                                        0.00000000e+00],
                                    [ 0.00000000e+00,  4.05877493e-02,  4.41144434e-01,
                                        -4.69359769e-02,  7.30804193e-01,  5.37605820e-02,
                                        6.84548202e-03],
                                    [ 0.00000000e+00,  9.99175978e-01, -1.79198260e-02,
                                        -9.98893006e-01, -3.64726576e-02,  9.98544984e-01,
                                        3.84639735e-03],
                                    [ 1.00000000e+00,  0.00000000e+00,  8.97257192e-01,
                                        3.12681371e-03, -6.81612043e-01,  4.20894763e-03,
                                        -9.99969172e-01]])
    ...
    def run_controller(self):
        ...
        # )
        position_ori_error = np.concatenate([np.array(position_error), np.array(ori_error)])
        vel_pos_ori_error = np.concatenate([np.array(vel_pos_error), np.array(vel_ori_error)])

        # x_r = ddot(x_d) + Kp*x_error + Kd*dot(x_error); we set ddot(x_d)=0
        kp = np.array([2500,2500,2500,2500,2500,2500])#800 2500 3600 5000
        kd = 2 * np.sqrt(kp) *1
        
        ddot_pos_ori = np.multiply(position_ori_error,kp) + np.multiply(
            vel_pos_ori_error, kd)
        ...
        J_full_vel =  20 * (self.J_full - self.J_full_last)
        self.J_full_last = self.J_full

        J_full_inertia_weighted = np.linalg.pinv(self.mass_matrix).dot(self.J_full.T).dot(lambda_full)

        self.torques = self.mass_matrix.dot(J_full_inertia_weighted).dot(ddot_pos_ori - J_full_vel.dot(self.joint_vel)) + self.torque_compensation
