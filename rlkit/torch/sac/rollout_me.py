...
def rollout(
    ...
    while path_length < max_path_length:
        ...
        #     o_modi=np.expand_dims(np.array(o),axis=0)
        #     batch_size,length = o_modi.shape
            
        #     image_batch = o_modi[:,:64*64*3].reshape((batch_size,3,64,64))
        #     from torchvision.utils import save_image
        #     from PIL import Image
        #     from torchvision import transforms

        #     image_batch = torch.Tensor(image_batch)
        #     proprio_batch = torch.Tensor(o_modi[:,64*64*3:])
        #     preprocess = transforms.Compose([transforms.Resize(256),transforms.CenterCrop(224),transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]) 
        #     image_batch = preprocess(image_batch)
        #     if torch.cuda.is_available():
        #         image_batch = image_batch.to('cuda')
        #         #proprio_batch = proprio_batch.to('cuda')
        #         model.to('cuda')
        #     with torch.no_grad():
        #         features = model(image_batch)
        #     features = features.to('cpu')
        #     h = torch.cat((features,proprio_batch),dim=1)
        if o.shape[0]== 4140:  #12331
            if input_channel ==3:
                model= torch.hub.load('pytorch/vision:v0.10.0', 'shufflenet_v2_x0_5', pretrained=True)
            else:
                #model= torch.hub.load('pytorch/vision:v0.10.0', 'shufflenet_v2_x0_5', pretrained=True)         
                ...
                PATH = '/homeL/wchen/Assembly_RPL/rlkit/rlkit/torch/sac/VanillaVAE/channel_32/checkpoints/last.ckpt'
            ...
            image_batch = o_modi[:,:64*64*1].reshape((batch_size,1,64,64))
            from torchvision.utils import save_image
            from PIL import Image
            from torchvision import transforms#transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
            #preprocess = transforms.Compose([transforms.Resize(256),transforms.CenterCrop(224),]) 
            #image_batch = preprocess(image_batch)
            image_batch = torch.Tensor(image_batch)
            proprio_batch = torch.Tensor(o_modi[:,64*64*1:])
            if torch.cuda.is_available():
                image_batch = image_batch.to('cuda')
                model.to('cuda')
            with torch.no_grad():
                #features = model(image_batch)
                result,input,mu,std = model(image_batch)
                features = torch.cat([mu, std],axis=-1)
            features = features.to('cpu')
            h = torch.cat((features,proprio_batch),dim=1)
        o_modi=h.numpy()
        a, agent_info = agent.get_action(o_modi)
        ...
        if video_writer is not None:
            ...
            #import mujoco_py
            #viewer = mujoco_py.MjRenderContextOffscreen(env, -1)
            #frame = viewer.render(256, 256, 0)
